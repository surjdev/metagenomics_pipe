...
| 2026-09-15 | Antigravity | Built dedicated Short vs Long Assembly-Free Metagenomics Comparison Pipeline (`main_compare_assembly_free.nf`, `conf/compare_assembly_free.config`, `bin/compare_assembly_free.py`, `modules/local/compare_assembly_free/main.nf`, `run_compare_assembly_free.sh`, `submit_compare_assembly_free.sbatch`) enabling side-by-side Illumina vs Nanopore profiling, Jaccard/Pearson/Spearman statistical concordance, top taxa differential matrix, Krona charts, BIOM 1.0 JSON & TSV matrix export (individual + combined multi-sample), and interactive HTML/Markdown reporting. |
| 2026-09-25 | Antigravity | Integrated exact SRA Run accessions (`SHORT_KEYS` / `LONG_KEYS`) for 6 hybrid subjects into `bin/pull_hybrid_data_fasterq.sh`, configured default skipping of `CD35` & `TD35` (already downloaded/installed), switched to `--split-3`, preserved existing samplesheet entries, added `bin/import_existing_hybrid_data.sh` to auto-compress and map files from `/home/koraop/nob_dir/metagenomics/metagenomic_Long_short`, and updated `submit_pull_hybrid_data.sbatch`. |
| 2026-09-25 | Antigravity | Fixed fasterq-dump output filename mismatch in `bin/pull_hybrid_data_fasterq.sh` (SRA Toolkit 2.10 creates `<acc>.sra_1.fastq` when passed a `.sra` file path, causing pigz to fail looking for `<acc>_1.fastq`). Implemented dynamic locator helpers (`locate_illumina_fastqs`, `locate_nanopore_fastq`) that auto-detect `.sra_1.fastq`, `_1.fastq`, and resume existing uncompressed FASTQ files on disk without re-extracting; fixed `--test` mode spot limit handling; verified 100% end-to-end with mock test suite. |
| 2026-09-26 | Antigravity | Created combined 6-sample samplesheet for Assembly-Free Comparison (`samplesheet_compare_assembly_free.csv` & updated `samplesheet_hybrid.csv`) unifying old raw reads from `metagenomic_Long_short` (`CD35`, `TD35`) with new downloaded reads from `test_data/hybrid_test` (`CD22`, `CD3`, `TD22`, `TD3`); created `bin/generate_samplesheet_assembly_free.sh`; made `submit_compare_assembly_free.sbatch` default to `raw` mode for full cohort processing. |
