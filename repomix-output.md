...
Generated by Repomix on: 2026-08-13 11:20:51
...
```````````markdown
This file is a merged representation of the entire codebase, combining all repository files into a single document.
Generated by Repomix on: 2026-08-10 06:19:28
...
``````````markdown
This file is a merged representation of the entire codebase, combining all repository files into a single document.
Generated by Repomix on: 2026-08-10 05:50:16
...
`````````markdown
This file is a merged representation of the entire codebase, combining all repository files into a single document.
Generated by Repomix on: 2026-08-10 05:39:03
...
````````markdown
This file is a merged representation of the entire codebase, combining all repository files into a single document.
Generated by Repomix on: 2026-08-10 05:09:25
...
```````markdown
This file is a merged representation of the entire codebase, combining all repository files into a single document.
Generated by Repomix on: 2026-08-10 05:01:53
...
```text
params {
    // 1. Core Pipeline Inputs & Outputs
    input           = null           // samplesheet CSV, required
    outdir          = 'results'
    normalise       = false          // gate the optional BBNorm step

    // 2. Host Removal (QC arms)
    host_bt2_index  = null
    host_fasta      = null

    // 3. Long-read processing (Dorado tool)
    dorado_model    = null           

    // 4. Assembly-Free Profiling (ตรงตาม 7 arguments ล่าสุดของคุณ)
    kraken2_db      = null
    bracken_db      = null
    metaphlan_db    = null
    humann_nuc_db   = null
    ...
    // 6. Bin Taxonomy & Annotation (ตรงตาม 5 arguments ล่าสุดของคุณ)
    gtdbtk_db       = null           // ส่งเข้า ch_tax_db
    cat_taxonomy    = null           // ส่งเข้า ch_cat_taxonomy
    mmseqs_db       = null           // ส่งเข้า ch_mmseqs_db
    eggnog_db       = null           // ส่งเข้า ch_eggnog_db
}
...
workflow {
    ...
    ch_samplesheet = channel.fromPath(params.input)
        ...
        .map { row ->
            def meta = [
                ...
                input_type: row.input_type ?: 'fastq'
            ]
            return [
                meta,
                row.fastq_1 ? file(row.fastq_1) : null,
                row.fastq_2 ? file(row.fastq_2) : null,
                row.long_read ? file(row.long_read) : null
            ]
        }

    // Split into a short-read channel [meta, r1, r2]
    ch_short_reads = ch_samplesheet
        .filter { it[1] != null }
        .map { meta, r1, r2, lr -> [ meta, r1, r2 ] }

    // Split into a long-read channel [meta, reads]
    ch_long_reads = ch_samplesheet
        .filter { it[3] != null }
        .map { meta, r1, r2, lr -> [ meta, lr ] }
    ...
    // 2. Fix the ASSEMBLY_FREE_PROFILING(...) call (Passing 5 arguments instead of 2)
    ASSEMBLY_FREE_PROFILING(
        SHORT_READ_QC.out.reads,
        LONG_READ_QC.out.reads,
        params.kraken2_db,
    ...
    // 3. Replace the RUN_SUMMARY call with plain positional arguments
    RUN_SUMMARY(
...
## modules/local/align_reads_to_contigs/main.nf
...
params {
    ...
    // NOTE: busco_lineage, checkm2_db, dorado_model, eggnog_db, gunc_db are also
    // referenced downstream but not yet declared here -- see Phase 5 of the roadmap.
}
